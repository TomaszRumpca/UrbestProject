...
public class Crossword extends Activity implements OnTouchListener,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Log.d("Murale", "Number of LinearLayout childs found: "+nextLineLayouts.size());
	...
	private ArrayList<EditText> getAllEditTexts(ArrayList<LinearLayout> nextLineLayouts){
	...
	private ArrayList<LinearLayout> getAllNextLineLayouts(RelativeLayout myRelativeLayout){
	...
	private void checkCrossword(){
		...
		saver.saveAchievedPoints(getApplicationContext(), TasksList.TASK_CROSSWORD, this.currentScore, DatabaseHandler.KEY_TASK_NAME);
	...
	private void checkSelectedLine(int selectedLine){
		...
		for(int i = 0; i< myEditTextList.size();i++){
			if(myEditTextList.get(i).getParent().equals(nextLineLayouts.get(selectedLine))){
				userAnswer = userAnswer + myEditTextList.get(i).getText().toString().toUpperCase(Locale.getDefault());
			}
		}
