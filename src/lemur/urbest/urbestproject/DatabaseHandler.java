...
public class DatabaseHandler {
	...
	private static final String CREATE_LOCATION_TABLE = "create table " + LOCATIONS_TABLE + " ("
	...
	private static final String CREATE_SCORE_TABLE = "create table " + SCORE_TABLE + " ("
			+ KEY_SCORE_ID + " " + SCORE_ID_OPTION + ", " + KEY_DATE_OF_COMPLETION + " " 
			+ DATE_OF_COMPLETION_OPTION + ", " + KEY_TASK_NAME + " " + TASK_NAME_OPTION + ", " 
			+ KEY_SCORE + " " + SCORE_OPTION + ");";
	
	private static final String CREATE_MARKERS_TABLE = "create table " + MARKER_TABLE + " ("
			+ KEY_MARKER_ID + " " + MARKER_ID_OPTION + ", " + KEY_MARKER_LATITUDE + " " 
			+ MARKER_LATITUDE_OPTION + ", " + KEY_MARKER_LONGITUDE + " " + MARKER_LONGITUDE_OPTION + ");";
	...
	public long update(String date, String task, float score, String whereClause, String[] whereArgs){
		ContentValues cv = new ContentValues();
		cv.put(KEY_DATE_OF_COMPLETION,date);
		cv.put(KEY_TASK_NAME, task);
		cv.put(KEY_SCORE, score);
	...
	public long update(double latitude, double longitude, String whereClause, String[] whereArgs){
		ContentValues cv = new ContentValues();
		cv.put(KEY_MARKER_LATITUDE, latitude);
		cv.put(KEY_MARKER_LONGITUDE,longitude);
	...
	public Cursor getAllEntries(String fromTable){
		...
		if(fromTable.equals(SCORE_TABLE)){
			String[] columns = {KEY_DATE_OF_COMPLETION,KEY_TASK_NAME, KEY_SCORE};
			return db.query(SCORE_TABLE, columns, null, null, null, null, KEY_DATE_OF_COMPLETION);
		}else if(fromTable.equals(LOCATIONS_TABLE)){
			String[] columns = {KEY_LATITUDE,KEY_LONGITUDE, KEY_DATE};
			return db.query(LOCATIONS_TABLE, columns, null, null, null, null, KEY_DATE);	
		}else if(fromTable.equals(MARKER_TABLE)){
			String[] columns = {KEY_MARKER_ID,KEY_MARKER_LATITUDE, KEY_MARKER_LONGITUDE};
			return db.query(MARKER_TABLE, columns, null, null, null, null, KEY_MARKER_ID);
		}else{
			return null;
		}
	}
